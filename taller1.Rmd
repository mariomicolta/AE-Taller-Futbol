...
Cargamos los datos utilizando el paquete readxl
...
Describimos los tipos de variable que contiene la base de datos y definimos la estructura de la misma.
...
En el resultado anterior podemos observar que la base de datos contiene **`r nrow(datos)`** registros y un total de **`r ncol(datos)`** variables.
...
Verificamos nuevamente los tipos de variable
```{r}
sapply(datos, class)
...
```
Identificamos un valor nulo en los goles marcados por el equipo local. Además se observan posibles datos atípicos.
...
Observamos que existen dos variables con errores de digitación.

Se procede a actualizarlas.
...
Debido a que el árbitro no debería ser un factor diferenciador, se borra como variable:

```{r}
final_data <- final_data %>% select(-Referee)
head(final_data)
```
